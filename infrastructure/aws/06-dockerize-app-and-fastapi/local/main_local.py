...
####### Local Model Loading Only (No Cloud Fallback) ##########

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
TRAINING_DIR = os.path.abspath(os.path.join(BASE_DIR, "../../02-model-training"))


def load_local_model(model_subdir: str, model_name: str):
    model_path = os.path.join(TRAINING_DIR, model_subdir, model_name)
    if not os.path.isdir(model_path) or not any(os.scandir(model_path)):
        raise FileNotFoundError(
            f"Model not found in local training directory: {model_path}"
        )
    print(f"Loading {model_name} from Local Training Dir: {model_path}")
    return model_path


path_sentiment = load_local_model(
    "sentiment_classification", "tinybert-sentiment-analysis"
)
sentiment_model = pipeline("text-classification", model=path_sentiment, device=device)

path_disaster = load_local_model(
    "disaster_tweets_classification", "tinybert-disaster-tweet"
)
social_media_model = pipeline("text-classification", model=path_disaster, device=device)

path_pose = load_local_model(
    "human_pose_classification", "vit-human-pose-classification"
)
